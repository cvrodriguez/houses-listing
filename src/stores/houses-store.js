...
export const useHousesStore = defineStore('houses-store', () => {
  ...
  // Getters
  const housesByzipAndCity = computed(() =>
    housesState.value.filter(
      (h) =>
        h.location.city.toLowerCase().includes(searchCriteria.value.toLowerCase()) ||
        h.location.zip.includes(searchCriteria.value)
    )
  )
